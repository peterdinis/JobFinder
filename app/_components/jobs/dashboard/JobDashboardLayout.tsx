...
import { experienceLevels, jobTypes, jobs, salaryRanges } from "./customData";
		...
		<>
			<div className=" bg-gray-50 dark:bg-background">
				<div className="pt-4 flex">
					<aside
						className={`fixed h-[calc(100vh-4rem)] bg-white dark:bg-stone-900 border-r border-gray-200 overflow-y-auto transition-all duration-300 ease-in-out ${sidebarWidth}`}
					>
						<button
						...
						<div
							className={`space-y-6 p-6 ${isSidebarCollapsed ? "hidden" : "block"}`}
						>
							{/* Job Type Filter */}
							<div>
								...
								<div className="space-y-2">
									{jobTypes.map((type) => (
										<label
											key={type}
											className="flex items-center p-2 hover:bg-stone-400 rounded-md cursor-pointer transition-colors"
										>
											<input
												type="radio"
												name="jobType"
											...
											/>
											<span className="ml-2 text-sm text-gray-600 dark:text-sky-50">
												{type}
											</span>
										</label>
									))}
					...
					{/* Main Content */}
					<main
						className={`${mainContentMargin} flex-1 p-8 transition-all duration-300 ease-in-out`}
					>
						{/* Search Section */}
						<div className="mb-8">
						...
						<div className="space-y-4">
							{filteredJobs.map((job) => (
								<div
									key={job.id}
									className="bg-white dark:bg-background rounded-xl shadow-sm border border-gray-200 dark:border-zinc-900 p-6 hover:shadow-md transition-all duration-200 group"
								>
									<div className="flex justify-between items-start">
										<div>
											...
											<p className="text-gray-600 mt-1 group-hover:text-gray-900 dark:text-sky-50 transition-colors">
												{job.company}
											</p>
									...
									<p className="mt-4 text-gray-600 dark:text-sky-50 line-clamp-2">
										{job.description}
									</p>
									<div className="mt-4 flex flex-wrap gap-2">
										{job.tags.map((tag) => (
											<span
												key={tag}
												className="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-full text-sm font-medium hover:bg-blue-100 transition-colors cursor-pointer"
											>
												{tag}
											</span>
										))}
